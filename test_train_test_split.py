...
print(f"Test period : {test_data.index[warmup_offset].date()} s.d. {test_data.index[-1].date()} (evaluasi mulai hari ke-{warmup_offset})")

print("\n=== HASIL DI TRAIN DATA ===")
train_result = run_backtest(train_data, min_agree=1.5, warmup=50, stop_loss_pct=0.03, take_profit_pct=0.06)
...
print("\n=== HASIL DI TEST DATA (dengan buffer 200 hari) ===")
test_result = run_backtest(test_data, min_agree=1.5, warmup=warmup_offset, stop_loss_pct=0.03, take_profit_pct=0.06)
